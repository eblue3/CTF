...
def create_device():
    ...
    device = {
        ...
        'PublicIP': request.json.get('PublicIP', ""),
...
# curl -i -H "Content-Type: application/json" -X POST -d '{"Name":"AtomPi4"}' http://172.16.70.132:3500/api/devices
...
def update_device(device_id):
    ...
    with open("PublicIP.txt", "w+") as publicipf:
        json.dump(devices, publicipf)
    return jsonify({'devices': device[0]})
# curl -i -H "Content-Type: application/json" -X PUT -d '{"PublicIP": "NewIP"}' http://172.16.70.132:3500/api/devices/2
